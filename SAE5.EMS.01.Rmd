...
```{r}
library(leaflet) 
# Si vous recevez une erreur ici, vous devez d'abord l'installer : install.packages("leaflet")

# Définir l'indicateur que vous voulez montrer dans le pop-up
indicateur_a_afficher <- "electricite"

# Définir la couleur et la taille des marqueurs (ajustez selon votre besoin)
couleur_marqueurs <- "red"
rayon_marqueurs <- 5 # Un rayon plus grand rend les points plus visibles

# --- Création de la Carte Leaflet ---
m <- leaflet(data = monde_data) %>%
  
  # 1. Ajouter le fond de carte (OpenStreetMap est un bon choix)
  addProviderTiles("OpenStreetMap.Mapnik") %>%
  
  # 2. Ajouter les marqueurs circulaires
  addCircleMarkers(
    ~LON, ~LAT, # Coordonnées à utiliser
    radius = rayon_marqueurs,
    color = couleur_marqueurs,
    fillOpacity = 0.8,
    stroke = FALSE, # Supprime le contour noir
    
    # 3. Définir le contenu du pop-up (ce qui apparaît au clic)
    popup = ~paste0(
      "<b>Pays:</b> ", NAME, "<br>",
      "<b>", indicateur_a_afficher, ":</b> ", electricite, "%"
    ),
    
    # 4. Définir le contenu du label (ce qui apparaît au survol)
    label = ~NAME,
