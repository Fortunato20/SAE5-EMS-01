...
# l'indicateur emissions_Co2 n' pas la meme longeur que les autres, donc on le rajoute apres 
```
...
Émissions de CO2 : moyenne élevée (571,4) masquée par une dispersion exceptionnelle(ecart-type : 3244), signe de profonds écarts entre pays pollueurs et non-pollueurs.
...
hist(data_t$emissions_co2, main = "Répartition des émissions de Co2", xlab = "Emissions de Co2",ylab="Effectif")
...
pays_selection <- c("France", "États-Unis", "Chine", 
                    "Algérie", "Maroc", "Tunisie", "Sénégal", 
                    "Côte d’Ivoire", "Nigeria", "Afrique du Sud", "Kenya", "Égypte")

# Filtrer uniquement ces pays
data_filtre <- subset(data_t, pays %in% pays_selection)

# Trier par espérance de vie décroissante
ord <- order(data_filtre$esperance_vie, decreasing = TRUE)

# --- Barplot ---
barplot(data_filtre$esperance_vie[ord],
        names.arg = data_filtre$pays[ord],
        las = 2, cex.names = 0.8,
        main = "Espérance de vie - Pays sélectionnés",
        ylab = "Esp. vie (ans)")

# --- Barplot ---
ord <- order(data_filtre$emissions_co2, decreasing = TRUE)

barplot(data_filtre$emissions_co2[ord],
        names.arg = data_filtre$pays[ord],
        las = 2,              # labels verticaux
        cex.names = 0.8,      # taille des noms
        main = "Émissions de Co2 - Pays sélectionnés",
        ylab = "Évolution depuis 1990 (%)")
```


# Boîte à moustache et diagramme de corrélation :
```{r}
...
# Boîte a moustache et diagramme de corrélation (Amélioré)
...
# Boite à moustache de tous les pays par indicateur et analyse croisée de certains indicateurs (2à2)
```{r}
library(dplyr)
...
# Analyse bivariée : Espérance de vie en X
x_var <- "esperance_vie"
...
ggplot(df_plot, aes_string(x = x_var, y = y_var, label = "pays")) +
  ...
  theme_minimal() +
  labs(
    title = paste("Analyse bivariée :", y_var, "en fonction de", x_var),
...
###############################################
# 1️⃣ Espérance de vie vs Accès à l'électricité
###############################################
...
# 3️⃣ Espérance de vie vs Taux de mortalité infantile (< 5 ans)
###############################################################
...
###############################################################
# 4️⃣ Émissions CO₂ vs Énergie renouvelable (pas d'espérance vie)
###############################################################
ggplot(df, aes(x = energie_renouv, y = emissions_co2, label = pays)) +
  ...
  geom_text_repel(size = 3) +
  labs(
    title = "Énergie renouvelable vs Émissions de CO₂",
    x = "Énergie renouvelable (% consommation totale)",
    y = "Émissions CO₂ (variation depuis 1990)"
  ) +
  theme_minimal()


################################################################
# 5️⃣ Croissance PIB vs Accessibilité à l'électricité (pas esp. vie)
################################################################
...
On peut faire un nuage entre l'axe 1 (capacité de faire  aux catastr) et le 4 ( l'exposition aux catastrophes). 
...
```{r}
# créer un data.frame propre (copier as.data.frame pour éviter les colonnes 'sf')
tmp <- as.data.frame(monde_data)[,13:23]
#str(tmp)          
#sapply(tmp, class) 

# garder seulement les colonnes numériques
is_num <- sapply(tmp, function(x) is.numeric(x) || is.integer(x))
donneesACP <- tmp[, is_num, drop = FALSE]  

iso_full <- as.character(monde_data$ISO3)   

#  garder uniquement les lignes sans NA dans donneesACP
garder_rows <- complete.cases(donneesACP)

iso_codes <- iso_full[garder_rows]
rownames(dACP) <- iso_codes
acp <- PCA(dACP, graph = FALSE)

# Représentation des variables (inchangée)
fviz_pca_var(acp, 
             col.var = "cos2", 
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE)

# ✅ Représentation des pays (individus) avec noms visibles pour les plus contributifs
fviz_pca_ind(acp,
             col.ind = "cos2", 
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             geom = "text",                   # remplacer "point" par "text" pour voir les noms
             select.ind = list(contrib = 20), # ne garder que les 20 plus contributifs
             repel = TRUE) 
# mieux avec ggplot pour les coordonnées (le nuage des points des individus), pas de cos², on peut deplacer les labels pour qu'ils soient plus visibles
```
...
C'est la carte leaflet pour presenté tout les pays ou on a la valeur de la hauteur moyenne
```{r}
library(leaflet) 
library(dplyr) # Nécessaire pour la fonction filter()
